...
// On change les chemins :
// - Accueil pointe vers la racine '/'
// - Contact pointe vers '/#contact' (L'ID de la section)
const links = [
  { name: 'Accueil', path: '/' },
...
// Fonction améliorée pour gérer l'état actif avec le hash
const isActive = (path: string) => {
  // Cas spécifique pour Contact (ancre)
  if (path.includes('#')) {
    return route.hash === '#contact';
  }

  // Cas pour Accueil : on veut qu'il soit actif uniquement si on est sur '/' SANS hash
  if (path === '/') {
    return route.path === '/' && !route.hash;
  }

  // Autres pages (Projets)
